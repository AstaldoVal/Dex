...
**Счётчик «только upstream»** (пути есть на `upstream/main`, нет в `HEAD` форка): после пакета 1 было 142; после пакета 2A — **108**; после пакета 2B — **98**; после пакета 2C — **76**; после пакета 2D — **73**; после пакета 2F — **39** (пересчёт: `comm -23` между `upstream/main` и `HEAD`).

**Следующие кандидаты (позже):** шаблоны vault `00-`…`07-` и `System/*` — по кластерам; для путей, общих с форком, только трёхстороннее сравнение, не `git checkout` всего каталога `core/`.
