...
function resolveLiveTealResumeId() {
  ...
  if (process.env.WORK_EXPERIENCE_E2E_RESUME_ID) {
    return String(process.env.WORK_EXPERIENCE_E2E_RESUME_ID).trim();
  }
...
function liveTealMode() {
  const v = (process.env.WORK_EXPERIENCE_E2E_LIVE || '').trim().toLowerCase();
  if (v === '0' || v === 'false' || v === 'skip' || v === 'off') return 'off';
  if (v === '1' || v === 'true' || v === 'force') return 'force';
  return 'auto';
}

function shouldRunWorkExperienceLiveTeal() {
