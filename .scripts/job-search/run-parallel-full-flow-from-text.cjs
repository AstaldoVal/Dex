 ...
 *   npm run job-search:full-flow-parallel -- -- path1.md path2.md
 */
...
async function main() {
  ...
  console.log(`Parallel full-flow: ${files.length} job(s), limitMs=${limitMs} stallMs=${stallMs}`);
  console.log('');

  const children = files.map((rel) => {
    const abs = path.isAbsolute(rel) ? rel : path.join(REPO_ROOT, rel);
    const slug = slugFromPath(abs);
    const logPath = path.join(REPO_ROOT, '00-Inbox/Job_Search/teal/parallel-full-flow', `${slug}.log`);
    const runKey = `ff-parallel-${slug}`;
