...
/**
 * Watchdog wrapper for Full Flow v2 (isolated from v1).
 *
 *   node full-flow-v2/run-full-flow-watched.cjs -- node full-flow-v2/run-full-linkedin-teal-flow.cjs [args…]
 */
const path = require('path');
const { getFullFlowPlan, formatPlanForChat } = require('../full-flow-stage-budgets.cjs');
const { TEAL_FLOW_V2_DIR, applyFlowV2Env } = require('./paths.cjs');
applyFlowV2Env();
...
const FLOW_STEP_END_RE = /\[@flow-step-v2\]\s+END\s+step=(\d+)\s+durationMs=(\d+)\s+pass=(true|false)/;
...
async function main() {
  ...
  if (!opts.cmd.length) {
    console.error('Usage: run-full-flow-watched.cjs -- node full-flow-v2/run-full-linkedin-teal-flow.cjs …');
    process.exit(1);
  }
  ...
  const runKey =
    ...
    (opts.cmd.join(' ').match(/--from-text\s+(\S+)/) ? `ff2-${path.basename(opts.cmd.find((a) => a.endsWith('.md')) || 'job')}` : `ff2-${process.pid}`);

  process.stderr.write(`[full-flow-v2-watch] ${formatPlanForChat(plan)}\n`);
  process.stderr.write(`[full-flow-v2-watch] limitMs=${limitMs} stallMs=${stallMs} runKey=${runKey}\n`);
    ...
    for (const line of s.split('\n')) {
      ...
      if (m) {
        ...
        process.stderr.write(
          `[full-flow-v2-watch] step ${m[1]} done ${Math.round(Number(m[2]) / 1000)}s pass=${m[3]}\n`
        );
      }
    }
  };
  ...
  const stallIv = setInterval(() => {
    ...
    if (Date.now() - lastOutputAt >= stallMs) {
      killed = true;
      reason = 'stall';
      process.stderr.write('[full-flow-v2-watch] stall — kill child\n');
      child.kill('SIGKILL');
    }
  }, STALL_POLL_MS);
  const limitIv = setInterval(() => {
    ...
    if (Date.now() - startedAt >= limitMs) {
      killed = true;
      reason = 'limit';
      process.stderr.write('[full-flow-v2-watch] limit — kill child\n');
      child.kill('SIGKILL');
    }
  }, STALL_POLL_MS);
  ...
  process.stderr.write(`[full-flow-v2-watch] child finished exit=${code} durationMs=${durationMs}\n`);
