...
const RUN = process.env.PAPERCLIP_RUN_ID || '8ad426d1-9e62-403a-abb0-8e15fd515aa8';
...
const IDEMPOTENCY = `cmo-pass-batch131-${RUN.slice(0, 8)}`;

const CMO_PASS_131 =
  '04-Projects/Applicator/docs/marketing/competitor-intel-heartbeat-2026-06-29-run131-cmo-pass.md';
const CM_TIMER_131 =
  '04-Projects/Applicator/docs/marketing/competitor-intel-heartbeat-2026-06-29-run131-timer.md';
const EVIDENCE = '04-Projects/Applicator/docs/evidence/HIR-cmo-pass-run131-2026-06-29.json';
const MIDDAY_INDEX =
...
const wp1 = await addWorkProduct(ISSUE, CMO_PASS_131, 'CMO PASS batch #131 (2026-06-29)');
const wp2 = await addWorkProduct(ISSUE, EVIDENCE, 'CMO PASS batch #131 evidence (2026-06-29)');
...
Community digest **одобрен** (CMO PASS). Batch **#131** timer (run \`15f57bc1\`) — **3** новых verified permalink vs #130 (\`1uivcl7\`, \`1uiv229\`, \`1uiusao\`). Два треда с **нулём комментариев** на момент ingest (\`1uivcl7\`, \`1uiv229\`). Competitor layer **carry-only**. Статусы gate W1 **не менялись**. Эпик [HIR-669](/HIR/issues/HIR-669) остаётся **blocked** до твоего первого **done** на любом gate W1-01…W1-06.
...
2. **Опционально Reddit** (без Accept): лид \`1uiql7m\`, затем batch #131 \`1uivcl7\` (career-break pivot), \`1uiv229\` (background-check ghosting), затем batch #130 \`1uitygb\`, \`1uitqt9\`. Paste-ready — \`${QUEUE}\`.
...
- CMO PASS batch #131: \`${CMO_PASS_131}\`
- CM timer archive: \`${CM_TIMER_131}\`
...
W1 execution pulse: CMO **PASS** batch #131 (\`${CMO_PASS_131}\`); **3** new community permalinks (\`1uivcl7\`, \`1uiv229\`, \`1uiusao\`); competitor carry-only vs #130; gates unchanged; epic [HIR-669](/HIR/issues/HIR-669) **blocked**.`,
    },
    runId: RUN,
  });
}
