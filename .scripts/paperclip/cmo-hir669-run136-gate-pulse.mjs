...
const RUN = process.env.PAPERCLIP_RUN_ID || '54182205-e2ba-4d89-ba88-893b21515b67';
...
const EVIDENCE = '04-Projects/Applicator/docs/evidence/HIR-cmo-gate-pulse-run136-2026-06-29.json';
const MIDDAY_INDEX =
...
const wp1 = await addWorkProduct(ISSUE, GATE_PULSE, 'CMO gate monitor pulse run136 (2026-06-29)');
const wp2 = await addWorkProduct(ISSUE, EVIDENCE, 'CMO gate pulse evidence run136 (2026-06-29)');
...
Новый дайджест Community Manager **ещё не пришёл** (последний — batch **#135**, CMO PASS на run \`f7f904f7\`). Эпик [HIR-669](/HIR/issues/HIR-669) по-прежнему **blocked**: твои gate W1-01…W1-06 в статусе **in_review**, нужен **done** на любом одном. Со стороны команды [HIR-647](/HIR/issues/HIR-647) (письмо waitlist) уже **done** — это **не** снимает gate; лендинг [HIR-70](/HIR/issues/HIR-70) всё ещё ждёт твоего **done**.
...
2. **Опционально Reddit** (без Accept): лид \`1uiql7m\`, тёплые \`1uiza1f\` / \`1uiz7u8\`; очередь — \`${QUEUE}\`.
...
W1 monitor pulse: no CM batch #136 yet; [HIR-647](/HIR/issues/HIR-647) **done** (agency); Roman gates **in_review**; epic [HIR-669](/HIR/issues/HIR-669) **blocked**.`,
    },
    runId: RUN,
  });
  h31CommentId = h31.id;
