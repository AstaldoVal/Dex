 ...
 * See .claude/reference/skill-prompt-injection.md
 *
 * Usage: node .scripts/skills/scan-skill-injection.cjs [--verbose] [--dir .claude/skills] [--no-comments] [--no-prereqs]
 ...
 *        node .scripts/skills/scan-skill-injection.cjs --check-name <skill-name>   # check name against known malicious list
...
/** In a Prerequisites/Installation block: dangerous remote-exec patterns (ClawHavoc-style). See clawhavoc-malicious-skills-reference.md */
...
/** Obfuscation / dynamic code execution (ClawHavoc, malware). Anywhere in file. */
...
/** Known malicious skill names (ClawHavoc and outliers). Use for --check-name or blocklist. See clawhavoc-malicious-skills-reference.md */
...
/** Optional: files/paths to skip (e.g. vendored or generated). */
...
function main() {
  ...
  if (checkNameIdx !== -1 && process.argv[checkNameIdx + 1]) {
    ...
    const normalized = name.replace(/^@?[\w-]+\//, ''); // strip org/ prefix if any
  ...
  console.log('Total:', allHits.length, 'hit(s). See .claude/reference/skill-prompt-injection.md');
  process.exit(1);
}
