#!/usr/bin/env bash
# Install Cursor MDC Link from VS Code Marketplace (Cursor uses Open VSX; extension may be missing there).
set -e

EXT_PUBLISHER="udit"
EXT_NAME="cursor-mdc-link"
TMP_DIR="${TMPDIR:-/tmp}/cursor-mdc-link-install"
VSIX_PATH="$TMP_DIR/mdc-link.vsix"
...
rm -rf "$TMP_DIR"
echo "Done. Cursor MDC Link installed. Reload Cursor window if links do not activate."
