...
3. **Rich progress bar:** по умолчанию **`--progress-format rich`** (и то же поведение у **`auto`**: полоса Rich на TTY или ASCII-бар с ETA при pipe/`2>log`, без переключения на **lines** по отсутствию TTY).
...
- **`--progress-format auto|rewrite|lines|rich`** — по умолчанию **`rich`**. **`rich`** и **`auto`** (алиас): полоса **Rich** на настоящем TTY **`stderr`**; если **`stderr`** не TTY (pipe, **`2>log`**, **`tee`**), используется **ASCII-полоса с ETA** (одна обновляемая строка в логе), а не режим **lines** и не голый **rewrite**. Режим **lines** (прореженные строки `[transcript-media] …% …`) только при явном **`--progress-format lines`**. **`rewrite`** — одна строка с `\r` без полосы. Полный план параметров пишется в **stderr**: на TTY — Rich-панели, в **файл** — plain-секции. **`tail -f log`** для живого прогресса. **`TRANSCRIPT_RICH=1`** / **`TRANSCRIPT_NO_RICH=1`** — как раньше.
