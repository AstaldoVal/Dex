...
9. **Save / export (optional)**  
   When the user wants the PRD in Google Docs or Google Spreadsheets: use Google Drive MCP to create a Doc or Sheet with the PRD content (`gdrive_create_doc` / `gdrive_create_sheet` if available) and share the link. If the Drive MCP is read-only, save to vault (or export to .docx) and instruct the user to copy into Google or upload to Drive.
