...
## Saving and exporting the artifact

After generating the PRD or spec doc:

1. **Save to vault** — Write the final PRD/spec to the project folder (e.g. `04-Projects/.../PRD.md`) or `00-Inbox/` as agreed.
2. **Save to Google Docs or Google Spreadsheets (when requested):** If the user asks to put the artifact in Google Docs or in a Google Sheet, or when handoff to stakeholders is via Google:
   - Use Google Drive MCP: if `gdrive_create_doc` / `gdrive_create_sheet` are available, create the document or spreadsheet with the artifact content and return the link.
   - If the Drive MCP is read-only, save the artifact to vault (or export to .docx if applicable), then tell the user: "Saved to [path]. To have it in Google Docs/Sheets, copy the content into a new Doc or upload the file in Drive; or we can extend the Drive MCP for direct create."

Offer explicitly: "Do you want this saved to a Google Doc or Google Sheet for sharing?" when the PRD/spec is finalized.
