...
**Example:** If `.mcp.json.example` has a new Python MCP block but the user's config does not:
```json
"example-mcp": {
  "type": "stdio",
  "command": "<vault_path>/.venv/bin/python",
  "args": ["<vault_path>/core/mcp/example_server.py"],
...
Add to summary if new MCPs were added: "✓ Added new MCP servers: [names]"
